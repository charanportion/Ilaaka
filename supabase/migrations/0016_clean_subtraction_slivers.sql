-- ST_Difference leaves topology slivers when two polygons "should" share an
-- edge but have sub-decimeter floating-point differences. Buffer the
-- subtractor outward by ~1m before subtracting so the cut completely covers
-- any precision-level gap. The buffer is invisible at typical zooms.
